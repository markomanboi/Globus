...
import com.example.globus.quizDB.QuizGlobalizationConcepts;
import com.example.globus.quizDB.QuizGlobalizationRegion;
...
public class QuizRegionalism extends AppCompatActivity {
    ...
    Button play_button;
    String get;
    QuizGlobalizationRegion quizGlobalizationRegion;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_quiz_regionalism);
        ...
        quizGlobalizationRegion = new QuizGlobalizationRegion(this);
        quizGlobalizationRegion.createDatabase();
        quizGlobalizationRegion.openDatabase();
        quizGlobalizationRegion.getWritableDatabase();
    ...
    public void onClick(View v) {//When this method is executed then there will be new question came and also same method for play button
        ...
        if (visibility == 0) {
            ...
            new CountDownTimer(120000, 1000) {//countdowntimer
                ...
                public void onFinish() {
                    ...
                    editor.putInt("Regionalism", l * 10).apply();
                    donutProgress.setProgress(0);
                    if(variable==0) {
                        Intent intent = new Intent(QuizRegionalism.this, QuizResults.class);
                        intent.putExtra("correct", l);
                        intent.putExtra("attemp", k);
                        intent.putExtra("categ", 6);
        ...
        if (get.equals("c1")) {
            ...
            if (j==10 || time==0) {
                ...
                countQuestion++;
                numQuestion.setText(countQuestion+"/"+10);
                Ques = quizGlobalizationRegion.readQuestion(list.get(j));
                Opta = quizGlobalizationRegion.readOptionA(list.get(j));
                Optb = quizGlobalizationRegion.readOptionB(list.get(j));
                Optc = quizGlobalizationRegion.readOptionC(list.get(j));
                Optd = quizGlobalizationRegion.readOptionD(list.get(j));
                global = quizGlobalizationRegion.readAnswer(list.get(j++));
            }
