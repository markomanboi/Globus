...
public class QuizUN extends AppCompatActivity {
    ...
    Button play_button;
    String get;
    QuizGlobalizationUN quizGlobalizationUN;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_quiz_u_n);
        ...
        quizGlobalizationUN = new QuizGlobalizationUN(this);
        quizGlobalizationUN.createDatabase();
        quizGlobalizationUN.openDatabase();
        quizGlobalizationUN.getWritableDatabase();
    ...
    public void onClick(View v) {//When this method is executed then there will be new question came and also same method for play button
        ...
        if (visibility == 0) {
            ...
            new CountDownTimer(120000, 1000) {//countdowntimer
                ...
                public void onFinish() {
                    ...
                    editor.putInt("UN and International Law", l * 10).apply();
                    donutProgress.setProgress(0);
                    if(variable==0) {
                        Intent intent = new Intent(QuizUN.this, QuizResults.class);
                        intent.putExtra("correct", l);
                        intent.putExtra("attemp", k);
                        intent.putExtra("categ", 5);
        ...
        if (get.equals("c1")) {
            ...
            if (j==10 || time==0) {
                ...
                Ques = quizGlobalizationUN.readQuestion(list.get(j));
                Opta = quizGlobalizationUN.readOptionA(list.get(j));
                Optb = quizGlobalizationUN.readOptionB(list.get(j));
                Optc = quizGlobalizationUN.readOptionC(list.get(j));
                Optd = quizGlobalizationUN.readOptionD(list.get(j));
                global = quizGlobalizationUN.readAnswer(list.get(j++));
            }
